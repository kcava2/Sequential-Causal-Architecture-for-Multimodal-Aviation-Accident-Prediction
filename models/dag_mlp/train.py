...
class HFACSCausalDAGMLP(nn.Module):
    """
    Three-step causal DAG-MLP following the Direct Parent Dependency rule.

    Step A: [Organizational Climate | Employment]  dim=2
              → predict A: Supervisory Conditions

    Step B: [WeatherCondition | TimeOfDay | SkyCondNonceil | Personnel | Supervisory]  dim=5
              → predict B: Operator Conditions

    Step C: [soft_B | Supervisory Conditions]  dim=n_B+1
              → predict C: Unsafe Acts

    Key improvement over the LSTM: no detach() calls — gradients from crit_C
    flow back through soft_B into MLP_B parameters. Each MLP is
    Linear(in, H) → ReLU → Linear(H, H) → ReLU with a separate output head.
    """

    STEP_A_SIZE = 2  # Organizational Climate (encoded), Employment (numerical)
    STEP0_SIZE  = 5  # Weather, TimeOfDay, SkyCondNonceil, Personnel, Supervisory
    ...
    def __init__(self, hidden_size, n_A, n_B, n_C, dropout=0.2):
        ...
        self.mlp_a = nn.Sequential(
            nn.Linear(self.STEP_A_SIZE, hidden_size),
            nn.ReLU(),
        ...
        self.mlp_b = nn.Sequential(
            nn.Linear(self.STEP0_SIZE, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
        )
        ...
        # Input: [soft_B (n_B) | GT_Supervisory (1)]
        self.mlp_c = nn.Sequential(
            nn.Linear(n_B + 1, hidden_size),
            nn.ReLU(),
            nn.Linear(hidden_size, hidden_size),
            nn.ReLU(),
        )
    ...
    def forward(self, step_a, step0):
        ...
        hA       = self.drop(self.mlp_a(step_a))
        logits_A = self.head_A(hA)

        # ── Step B → predict B (Operator Conditions) ──────────────────────────
        hB       = self.drop(self.mlp_b(step0))
        logits_B = self.head_B(hB)
        soft_B   = torch.softmax(logits_B, dim=1)  # no detach — gradients flow back

        # ── Step C → predict C (Unsafe Acts) ──────────────────────────────────
        # Supervisory Conditions (step0[:, 4]) mirrors the LSTM's GT supervisory input
        supervisory = step0[:, 4:5]
        step_c_in   = torch.cat([soft_B, supervisory], dim=1)
        hC          = self.drop(self.mlp_c(step_c_in))
        logits_C    = self.head_C(hC)
...
def train_model(
    ...
    """
    Train HFACSCausalDAGMLP and return (model, history).
    ...
    dropout      : float
    epochs       : int
    patience     : int — early stopping patience on val focal loss
    device       : torch.device
    verbose      : bool

    Returns
    -------
    model   : trained HFACSCausalDAGMLP (best checkpoint if val_loader provided)
...
def main():
    ...
    print("=" * 60)
    print("DAG-MLP Causal Architecture — Input/Output per Step")
    print("=" * 60)
    ...
    print("  Inputs : [soft predictions from Step B]")
    print("           Supervisory Conditions")
    ...
    plt.tight_layout()
    plot_path = os.path.join(os.path.dirname(__file__), "..", "..", "figures", "dag_mlp_training_curves.png")
